...
class EclecticIQ_api:
    ...
    def get_outh_token(self, test_credentials=True):
        ...
        try:
            ...
            if test_credentials:
                r = requests.get(self.baseurl + "/api", headers=self.headers, verify=self.verify_ssl, proxies=self.proxies, timeout=300)
    ...
    def send_api_request(self, method, path, params=None, data=None):
        ...
        try:
            if method == "post":
                r = requests.post(
                    url, headers=self.headers, params=params, data=json.dumps(data), verify=self.verify_ssl, proxies=self.proxies, timeout=300
                )
            elif method == "put":
                r = requests.put(
                    url, headers=self.headers, params=params, data=json.dumps(data), verify=self.verify_ssl, proxies=self.proxies, timeout=300
        ...
        if r and r.status_code in [100, 200, 201, 202, 204]:
            ...
            if r is None:
                ...
                msg = f"Could not perform request to EclecticIQ VA: {method}: {url}. Status code:{r.status_code}"
                self.eiq_logging.exception(msg)
                raise Exception(msg)
    ...
    def get_source_group_uid(self, group_name):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["groups"], params="filter[name]=" + str(group_name))
    ...
    def get_user_status(self):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["user_status"])
    ...
    def create_incoming_feed(
        ...
        r = self.send_api_request("post", path=API_PATHS[self.eiq_api_version]["incoming_feeds"], data=create_feed_dict)
    ...
    def download_incoming_feed(self, feed_id, feed_provider_task):
        ...
        r = self.send_api_request(
            "post", path=API_PATHS[self.eiq_api_version]["tasks"] + str(feed_provider_task) + "/run", data=run_task_download_feed
        )
    ...
    def delete_outgoing_feed(self, feed_id):
        ...
        self.send_api_request("delete", path=API_PATHS[self.eiq_api_version]["outgoing_feeds"] + str(feed_id))
    ...
    def delete_incoming_feed(self, feed_id):
        ...
        self.send_api_request("delete", params="with_linked_data=true", path=API_PATHS[self.eiq_api_version]["incoming_feeds"] + str(feed_id))
    ...
    def delete_data_set(self, data_set_id):
        ...
        self.send_api_request("delete", path=API_PATHS[self.eiq_api_version]["dataset"] + str(data_set_id))
    ...
    def create_dataset(self, title, search_query):
        ...
        create_dataset = {"data": {"is_dynamic": True, "name": title, "search_query": search_query, "workspaces": [1]}}

        r = self.send_api_request("post", path=API_PATHS[self.eiq_api_version]["dataset"], data=create_dataset)
    ...
    def create_outgoing_feed(self, content_type, intel_set_id, feed_title, transport_type, update_strategy, access_group_name):
        ...
        create_outgoing_feed = {
            "data": {
                ...
                "transport_configuration": {"authorized_groups": [authorized_group_order_id], "is_public": False},
                "transport_type": transport_type,
        ...
        r = self.send_api_request("post", path=API_PATHS[self.eiq_api_version]["outgoing_feeds"], data=create_outgoing_feed)
    ...
    def enable_outgoing_feed(self, outgoing_feed_full_info):
        ...
        self.eiq_logging.info("Enable Outgoing Feed id={}".format(outgoing_feed_full_info["id"]))
        ...
        r = self.send_api_request(
            "put", path=API_PATHS[self.eiq_api_version]["outgoing_feeds"] + str(outgoing_feed_full_info["id"]), data=enable_outgoing_feed_payload
        )
    ...
    def get_task_status(self, task_id):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["task_status"] + task_id)
    ...
    def get_feed_info(self, feed_ids):
        ...
        for k in feed_ids:
            ...
            if not r.json()["data"]:
                self.eiq_logging.error("Feed id={0} information cannot be requested. Received response:" + str(r.json())).format(k)
                ...
                result.append(feed_result)
                self.eiq_logging.debug(f"Feed id={k} information retrieved successfully. Received response:" + str(json.dumps(feed_result)))
    ...
    def get_feed_content_blocks(self, feed, feed_last_run=None):
        ...
        if feed["packaging_status"] == "SUCCESS" and feed["update_strategy"] == "REPLACE":
            ...
            r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["feed_content_blocks"] + "{}/runs/latest".format(feed["id"]))
            ...
            r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["feed_content_blocks"] + "{}".format(feed["id"]) + "/")

            data = r.json()["data"]["content_blocks"]

            if (feed["created_at"] != feed_last_run["created_at"]) or feed_last_run["last_ingested"] is None:
                self.eiq_logging.info(
                    "Received list contains {} blocks for {} feed:{}. Feed created time changed or first run, "
                ...
                try:
                    ...
                    self.eiq_logging.info(
                        "Received list contains {} blocks for {} feed:{}.".format(len(diff_data), feed["update_strategy"], feed["id"])
                    )
                    return diff_data
                except ValueError:
    ...
    def get_group_name(self, group_id):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["group_id_search"] + str(group_id))
    ...
    def get_observable_by_id(self, id):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["observable_search"] + "/" + str(id))
    ...
    def lookup_observable(self, value, type=None):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["observable_search"], params=params)
    ...
    def batch_delete_observables(self, observables_list):
        ...
        r = self.send_api_request("delete", path=API_PATHS[self.eiq_api_version]["observables_batch_delete"], data=observables_to_delete)
    ...
    def delete_entity(self, entity_id):
        ...
        self.send_api_request("delete", path=API_PATHS[self.eiq_api_version]["entities"] + str(entity_id))
    ...
    def get_taxonomy_dict(self):
        ...
        r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["taxonomy_get"], params={"limit": 500})
    ...
    def get_entity_realtionships(self, source_id=None, target_id=None):
        ...
        if len(parsed_response["data"]) > 0:
            for i in parsed_response["data"]:
                ...
                if direction == "source":
                    r = self.send_api_request(
                        "get", path=API_PATHS[self.eiq_api_version]["entity_search"] + "/" + str(extract_uuid_from_url(i["data"]["target"]))
                    )
                elif direction == "target":
                    r = self.send_api_request(
                        "get", path=API_PATHS[self.eiq_api_version]["entity_search"] + "/" + str(extract_uuid_from_url(i["data"]["source"]))
                    )
    ...
    def get_entity_by_id(self, entity_id, observables_lookup=True, relationships_lookup=True):
        ...
        try:
            r = self.send_api_request("get", path=API_PATHS[self.eiq_api_version]["entity_search"] + "/" + str(entity_id))
            parsed_response = json.loads(r.text)
    ...
    def elastic_search(self, search_payload=None, latency_check=False, page_size=10):
        ...
        r = self.send_api_request("post", path=API_PATHS[self.eiq_api_version]["entity_search"] + "?size=" + str(page_size), data=search_payload)
            ...
            "data": {
                "data": {
                    ...
                    "security_control": {
                        ...
                        "time": {"type": "time", "start_time": today_begin, "start_time_precision": "second"},
                    },
                },
                "meta": {
